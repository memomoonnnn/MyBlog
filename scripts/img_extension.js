...
hexo.extend.filter.register('after_post_render', function (data) {
	...
	data.content = data.content.replace(regex, function (match, preAttributes, altText, width, postAttributes) {
		...
		if (desc) {
			...
			return `
        <figure class="hexo-img-ext-container with-caption" style="width: ${widthPx}px;">
      ...
      `;
			...
			// 场景 B: 无描述 -> 仅圆角图片 + 宽度控制
			return `
        <div class="hexo-img-ext-container" style="width: ${widthPx}px;">
            ${imgTag}
        </div>
      `;
		}
...
hexo.extend.injector.register('head_end', () => {
	const cssPath = path.join(hexo.base_dir, 'css_source', 'img_extension.css');
